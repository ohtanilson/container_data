...
traffic_amount_container_transportation_cost_and_profitability <-
  readxl::read_excel("input/traffic_amount_container_transportation_cost_and_profitability.xlsx") %>%
  tidyr::tibble()
traffic_amount_the_container_crisis_1982 <-
  readxl::read_excel("input/traffic_amount_the_container_crisis_1982.xlsx") %>%
  tidyr::tibble()
traffic_amount_world_sea_trades <-
  readxl::read_excel("input/traffic_amount_world_sea_trades.xlsx") %>%
  tidyr::tibble()
traffic_amount_review_of_maritime_transport <-
  readxl::read_excel("input/traffic_amount_review_of_maritime_transport.xlsx") %>%
  tidyr::tibble()
containerization_international_1973 <-
  readxl::read_excel("input/containerization_international_1973.xlsx") %>%
  tidyr::tibble()
...
conversion_ratio_million_tons_to_1000TEU =
  1000000/(conversion_ratio_tons_to_TEU*1000)
# construct shipping quantity dataset ----
ratio_trade_1985 <-
  traffic_amount_world_sea_trades %>%
  dplyr::filter(year == 1985) %>%
  dplyr::mutate(ratio_transpacific_eastbound_to_westbound =
...
containerization_international_1973_before_1970 <-
  containerization_international_1973 %>%
  dplyr::filter(built_year <= 73) %>%
  dplyr::mutate(
    route = dplyr::case_when(service == "Europe/ECNA" ~ "transatlantic",
...
first_asia_to_europe_route <-
  containerization_international_1973_before_1970 %>%
  dplyr::filter(route == "asia_and_europe") %>%
  dplyr::arrange(built_year)

containerization_international_1973_before_1970 <-
  containerization_international_1973_before_1970 %>%
  dplyr::group_by(built_year,
                  route) %>%
  dplyr::summarise(route_level_capacity_TEU =
                     sum(capacity_TEU)) %>%
  dplyr::ungroup()
...
containerization_international_1973_before_1970_transatlantic <-
  containerization_international_1973_before_1970 %>%
  dplyr::filter(route == "transatlantic") %>%
  dplyr::right_join(year_list,
                    by = c("built_year" = "built_year")) %>%
  dplyr::arrange(built_year) %>%
  dplyr::mutate(
    route_level_capacity_TEU = ifelse(
      is.na(route_level_capacity_TEU) == 1,
      0,
      route_level_capacity_TEU
    )
  ) %>%
  dplyr::mutate(route = "transatlantic") %>%
  dplyr::mutate(cumulative_route_level_capacity_TEU =
                  cumsum(route_level_capacity_TEU))

containerization_international_1973_before_1970_transpacific <-
  containerization_international_1973_before_1970 %>%
  dplyr::filter(route == "transpacific") %>%
  dplyr::right_join(year_list,
                    by = c("built_year" = "built_year")) %>%
  dplyr::arrange(built_year) %>%
  dplyr::mutate(
  ...
  dplyr::mutate(route = "transpacific") %>%
  dplyr::mutate(cumulative_route_level_capacity_TEU =
                  cumsum(route_level_capacity_TEU))
...
containerization_international_1973_before_1970_asia_and_europe <-
  ...
  dplyr::mutate(route = "asia_and_europe") %>%
  dplyr::mutate(cumulative_route_level_capacity_TEU =
                  cumsum(route_level_capacity_TEU))

## merge datasets
containerization_international_1973_before_1970 <-
  rbind(containerization_international_1973_before_1970_transatlantic,
        containerization_international_1973_before_1970_transpacific,
        containerization_international_1973_before_1970_asia_and_europe) %>%
  dplyr::filter(built_year >= 66) %>%
  dplyr::mutate(capacity_1000TEU = 
                  cumulative_route_level_capacity_TEU/1000) %>%
  dplyr::group_by(built_year) %>%
  dplyr::mutate(year_level_total_capacity_1000TEU =
                  sum(capacity_1000TEU)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(total_capacity_1000TEU_share =
                  capacity_1000TEU/year_level_total_capacity_1000TEU) %>%
  dplyr::mutate(built_year = built_year + 1900) %>%
  dplyr::select(built_year,
                route,
                capacity_1000TEU) %>%
  tidyr::pivot_wider(names_from = route,
              values_from = capacity_1000TEU)

traffic_amount <-
  containerization_international_1973_before_1970 %>%
  dplyr::left_join(traffic_amount_the_container_crisis_1982,
                   by = c("built_year" = "year")) %>%
  # assume that eastbound and westbound has equally shipping quantity (1/2)
  dplyr::mutate(transpacific_eastbound =
                  north_america*
                  (transpacific/(transatlantic+transpacific))*
                ...
                transatlantic_eastbound =
                  west_europe*
                  (transatlantic/(transatlantic+transpacific))*
                ...
                europe_to_asia =
                  far_east*
                  (asia_and_europe/(asia_and_europe+transpacific))*
  ...
  tidyr::replace_na(list(europe_to_asia = 0))
# traffic_amount_1970 <-
#   traffic_amount %>%
#   dplyr::filter(built_year == 1970)
# traffic_amount_1970_interpolate <-
#   traffic_amount %>%
#   dplyr::mutate(transpacific_eastbound_1970 = traffic_amount_1970$transpacific_eastbound,
#                 transpacific_westbound_1970 = traffic_amount_1970$transpacific_westbound,
#                 transatlantic_eastbound_1970 = traffic_amount_1970$transatlantic_eastbound,
#                 transatlantic_westbound_1970 = traffic_amount_1970$transatlantic_westbound,
#                 europe_to_asia_1970 = traffic_amount_1970$europe_to_asia,
#                 asia_to_europe_1970 = traffic_amount_1970$asia_to_europe)
...
# ### compute 1974 based on the_container_crisis_1982
# tranpacific_ratio_1974 = 
#   traffic_amount_the_container_crisis_1982$north_america[2]/
#   traffic_amount_the_container_crisis_1982$north_america[1]
# tranatlantic_ratio_1974 = 
#   traffic_amount_the_container_crisis_1982$west_europe[2]/
#   traffic_amount_the_container_crisis_1982$west_europe[1]
# asia_and_europe_ratio_1974 = 
#   traffic_amount_the_container_crisis_1982$far_east[2]/
#   traffic_amount_the_container_crisis_1982$far_east[1]
# traffic_amount_1966_1974 <-
#   traffic_amount_1966_1973 %>% 
#   dplyr::add_row(
#     year = 1974,
#     transpacific_eastbound = 
#       tranpacific_ratio_1974*traffic_amount_1970$transpacific_eastbound,
#     transpacific_westbound = 
#       tranpacific_ratio_1974*traffic_amount_1970$transpacific_westbound,
#     transatlantic_eastbound = 
#       tranatlantic_ratio_1974*traffic_amount_1970$transatlantic_eastbound,
#     transatlantic_westbound = 
#       tranatlantic_ratio_1974*traffic_amount_1970$transatlantic_westbound,
#     europe_to_asia = 
#       asia_and_europe_ratio_1974*traffic_amount_1970$europe_to_asia,
#     asia_to_europe = 
#       asia_and_europe_ratio_1974*traffic_amount_1970$asia_to_europe
#   )
...
traffic_amount_container_transportation_cost_and_profitability <-
  traffic_amount_container_transportation_cost_and_profitability %>%
  dplyr::filter(is.na(asia_and_europe) == 0) %>%
  dplyr::mutate(transpacific_eastbound =
                  transpacific*ratio_trade_1985$ratio_transpacific_eastbound_to_westbound,
                transpacific_westbound =
                  transpacific*(1-ratio_trade_1985$ratio_transpacific_eastbound_to_westbound),
                transatlantic_eastbound =
                  transatlantic*(ratio_trade_1985$ratio_transatlantic_eastbound_to_westbound),
                transatlantic_westbound =
                  transatlantic*(1-ratio_trade_1985$ratio_transatlantic_eastbound_to_westbound),
                europe_to_asia =
                  asia_and_europe*(ratio_trade_1985$ratio_europe_to_asia_to_asia_to_europe),
                asia_to_europe =
                  asia_and_europe*(1-ratio_trade_1985$ratio_europe_to_asia_to_asia_to_europe)
  ) %>%
  dplyr::select(-Source,
                -unit,
                -transpacific,
...
traffic_amount_container_transportation_cost_and_profitability[,2:7] <-
  FreqProf::approxm(
    traffic_amount_container_transportation_cost_and_profitability[,2:7],
    length(traffic_amount_container_transportation_cost_and_profitability$year))

## create data between 1985 and 1997 ----------
traffic_amount_world_sea_trades <-
  traffic_amount_world_sea_trades %>%
  dplyr::filter(is.na(europe_to_asia) == 0) %>%
  dplyr::select(-Source,
                -unit)
...
# traffic_amount_world_sea_trades <-
#   traffic_amount_world_sea_trades %>%
#   dplyr::filter(year > 1990) # drop year duplicated across datasets
...
traffic_amount_1973_1975[,2:7] <-
  FreqProf::approxm(traffic_amount_1973_1975[,2:7],
                    length(traffic_amount_1973_1975$year)) 
traffic_amount_1974 <-
  traffic_amount_1973_1975 %>% 
...
traffic_amount_1966_1997_converted <-
  ...
  dplyr::filter(year < 1995) # drop years duplicated across datasets
### merge quantity data  ----
...
saveRDS(container_shipping_quantity_each_route,
        file = "cleaned/container_shipping_quantity_each_route.rds")